boot<-function(boot){
  ...
  X<-t(rna)
  X<-X[,apply(X[tissue=="Tumor",],2,sd)>1E-1 & apply(X[tissue=="Normal",],2,sd)>1E-1]
  
  geneID<-rownames(rna)
  save(geneID,file=paste0("/sc/orga/projects/PGDAC/francesca/TSNet/Results/ccRCC/global/geneID.rda"))
  ...
  P<-as.numeric(purity); P[P<.01]<-.01; P[P>.99]<-.99
     ...
     set.seed(boot)
     index<-sample(class,replace=FALSE)
     
     X1<-X[index==1,]; P1<-P[index==1]
     X2<-X[index==2,]; P2<-P[index==2]
     ...
     save(out1,out2,bulk1,bulk2,file=paste0("/sc/orga/projects/PGDAC/francesca/TSNet/Results/ccRCC/global/Permutation_global_",boot,".rda"))
}